...
object JioSaavnClient {
    ...
    suspend fun searchSongs(query: String, limit: Int = 5): List<JioSaavnSongRaw> {
        ...
        return try {
            val rawText: String = client.get("/api.php") {
            ...
            val response: JioSaavnSearchResponse = jsonConfig.decodeFromString(rawText)
            response.results
        } catch (e: Exception) {
            timber.log.Timber.e(e, "JioSaavn search failed for query: $query")
            e.printStackTrace()
            emptyList()
        }
    }
