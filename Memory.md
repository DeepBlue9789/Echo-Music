...
### F. Elimination of Late Buffer_Ready & Spurious BUFFER_WAIT Freezes
- **Server Guard Against Late buffer_ready**: In `P2PWebSocketServer.kt`, `handleBufferReady` immediately returns if `currentBufferingTrackId == null || virtualTimelineRate > 0.0`. Once playback has been scheduled/started, late `buffer_ready` packets cannot initiate a false buffer barrier or broadcast `BUFFER_WAIT`.
- **Active Playback Spurious BufferWait Rejection**: In `ListenTogetherManager.kt`, `ListenTogetherEvent.BufferWait` checks if `connection.player.isPlaying && timelineRate > 0.0`. If music is actively playing, spurious `BUFFER_WAIT` events are ignored and **never call `connection.pause()`**, preventing the 7-second room freeze.
- **Robust Auto-Transition Track ID Normalization**: In `onMediaItemTransition`, both `mediaItem.mediaId` and `currentMetadata?.id` are normalized via `normalizeTrackId`, ensuring prefix-formatted IDs (e.g. `album_xxx/CTQvEx3ajJA`) never block transition handling.
- **Guest Auto-Transition Identity Check**: In `PlaybackActions.CHANGE_TRACK`, guest checks `normalizeTrackId(player.currentMediaItem?.mediaId) == normalizeTrackId(track.id)`. If matched, the guest skips `applyPlaybackState`, never seeks to 0:00, never pauses, and continues playing gaplessly.

### G. Reconnect State Recovery & Position Preservation on Disconnect
- **Accurate Position on Peer Disconnect**: In `P2PWebSocketServer.kt` `onClose`, when pausing playback on peer leave, the server calculates real elapsed position from `virtualTimelineRefPos + (now - refTime) / 1000.0` instead of using the stale un-updated `_roomState.value.position` (which was 0L). The exact millisecond position is preserved for when the peer reconnects.
- **Fresh SeqId Reset on JoinApproved**: In `ListenTogetherManager.kt` `JoinApproved`, `lastAppliedSeqId` is reset to `-1`. In `handleSessionSnapshot`, `lastAppliedSeqId >= 0 && payload.seqId < lastAppliedSeqId` ensures that newly joined/reconnected peers never discard the initial `SESSION_SNAPSHOT` due to stale sequence numbers from prior sessions.
- **Normalized Track Comparison on Reconnect Snapshot**: In `handleSessionSnapshot`, `normalizeTrackId` is applied to both `player.currentMediaItem?.mediaId` and `payload.trackId ?: payload.trackInfo?.id` so track differences are accurately detected and loaded on reconnect.

### H. Instant Initial Track Synchronization & P2P Reconnection
- **Server Pre-Seeding on Host Launch**: In `P2PWebSocketServer.kt`, `seedInitialState(track, isPlaying, positionMs, queue)` populates `_roomState` and the virtual timeline immediately upon hosting. When a partner connects, the server delivers an accurate `SESSION_SNAPSHOT` on message 1, syncing the song, queue, and live playback timestamp instantly.
- **Protection Against Track Resets on Peer Join**: In `ListenTogetherManager.kt` `UserJoined`, host checks `roomState.value?.currentTrack == null` before sending `sendTrackChangeInternal`. If the host is already playing, it never triggers a room-wide `CHANGE_TRACK` reset to 0:00.
- **P2P Auto-Reconnection Unlocked**: In `ListenTogetherClient.kt`, removed early return blocks that previously aborted reconnect loops for P2P sessions. Reconnections now correctly re-invoke `connectDirect(directTargetUrl, username)`. In `P2PPartnerManager.kt`, `startAutoReconnectGuardian` checks for both `ERROR` and `DISCONNECTED` states.
- **Cloud Fallback Isolation & Protobuf Guard**: Guarded `sendClockSyncRequest` against protobuf mode and ensured `storedRoomCode == "P2P-MESH"` never sends cloud `RECONNECT` requests to the public server.

### I. Room State Reset, Reconnect Unblocking & Background Auto-Discovery
- **Elimination of Self-Room Connection**: In `P2PPeerDiscovery.kt`, tracked local `currentDeviceName` and filtered out all local device IP addresses (loopback, LAN, Tailscale) and matching device name during mDNS resolution and peer caching. In `P2PPartnerManager.kt` and `P2PPartnerSection.kt`, prevented blank default to `127.0.0.1` and added self IP checking on connect.
- **Fresh Standby Room Instance on Disconnect**: In `P2PWebSocketServer.kt`, added `resetRoomState()` which clears `peerSessions`, `bufferedUserIds`, resets timeline ($t_{\text{ref}} = \text{now}, p_{\text{ref}} = 0.0, r = 0.0$), sequence numbers, and restores `_roomState` to an empty standby state when all peers exit or a new session is hosted. Removed synthetic `"host-local"` user injection that caused `hostId` desynchronization. Added explicit handling for `MessageTypes.LEAVE_ROOM`.
- **Host Client Reconnection Unblocked**: In `P2PPartnerManager.kt`, cleared `isIntentionalDisconnect` when peers join so the host's client can always reconnect to its local server after previous manual disconnects.
- **Guest Exit from Trapped In-Room UI**: In `ListenTogetherClient.kt`, skipped persisting ephemeral `P2P-MESH` sessions to DataStore. On P2P reconnection timeout/failure, cleared `_roomState.value = null` and reset role to `NONE`, allowing the guest to cleanly return to the connection UI.
