                                ...
                                AnimatedActionButton(
                                    ...
                                    onClick = {
                                        if (isDownloadComplete) {
                                            ...
                                            val urlToDownload = currentStatus.apkUrl ?: "https://github.com/DeepBlue9789/Echo-Music/releases/download/${currentStatus.version}/echomusic.apk"
...
fun isNewerVersion(latestVersion: String, currentVersion: String): Boolean {
    ...
    if (latestVersionClean.equals(currentVersionClean, ignoreCase = true)) return false

    val latestNumeric = latestVersionClean.split("-").first().split("+").first().trim()
    val currentNumeric = currentVersionClean.split("-").first().split("+").first().trim()

    val latestParts = latestNumeric.split(".").map { it.toIntOrNull() ?: 0 }
    val currentParts = currentNumeric.split(".").map { it.toIntOrNull() ?: 0 }
...
suspend fun checkForUpdate(
    ...
    withContext(Dispatchers.IO) {
        try {
            val url = URL("https://api.github.com/repos/DeepBlue9789/Echo-Music/releases/latest")
            ...
            if (shouldShow) {
                ...
                try {
                    val changelogUrl =
                        URL("https://github.com/DeepBlue9789/Echo-Music/releases/download/$tagWithPrefix/changelog.json")
...
suspend fun fetchChangelogForVersion(currentVersion: String): WhatsNewInfo? = withContext(Dispatchers.IO) {
    try {
        ...
        val releasesJson = try {
            openTimedStream("https://api.github.com/repos/DeepBlue9789/Echo-Music/releases")
        ...
        var description: String? = null
        try {
            val changelogJson = try {
                openTimedStream("https://github.com/DeepBlue9789/Echo-Music/releases/download/$tag/changelog.json")
                    .bufferedReader().use { it.readText() }
            } catch (e: Exception) {
                openTimedStream("https://github.com/EchoMusicApp/Echo-Music/releases/download/$tag/changelog.json")
                    .bufferedReader().use { it.readText() }
            }
            val changelogData = JSONObject(changelogJson)
            description = changelogData.optString("description").takeIf { it.isNotEmpty() }
